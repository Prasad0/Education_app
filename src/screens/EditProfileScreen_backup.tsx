...
const EditProfileScreen: React.FC<EditProfileScreenProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    if (actualProfile) {
      ...
      setFormData({
        ...
        stream: actualProfile.stream || '',
        current_standard: actualProfile.current_standard || '',
        board: actualProfile.board || '',
        current_school: actualProfile.current_school || '',
        target_exams: Array.isArray(actualProfile.target_exams) ? actualProfile.target_exams : [],
  ...
  const toggleTargetExam = (exam: string) => {
    setFormData(prev => {
      const currentExams = [...prev.target_exams];
      if (currentExams.includes(exam)) {
        return { ...prev, target_exams: currentExams.filter(e => e !== exam) };
      } else {
        return { ...prev, target_exams: [...currentExams, exam] };
      }
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (actualProfile?.user_type === 'student') {
        profilePayload.stream = formData.stream || null;
        profilePayload.current_standard = formData.current_standard || null;
        profilePayload.board = formData.board || null;
        profilePayload.current_school = formData.current_school || null;
        profilePayload.target_exams = formData.target_exams;
      }
